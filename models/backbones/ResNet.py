...
class ResNet(nn.Module):
    ...
    def __init__(self, block, n_blocks, drop_rate=0.0, dropblock_size=5, max_pool=True,part=False):
    ...
    def forward(self, x, is_feat=False):   # x:[3,84,84]
        l1 = self.layer1(x)   # [64,42,42]
        l2 = self.layer2(l1)  # [160,21,21]
        l3 = self.layer3(l2)  # [320,10,10]
        l4 = self.layer4(l3)  # [640,5,5]
...
def resnet12(drop=False, part=False,drop_rate=0.1, dropblock_size=5, max_pool=True, **kwargs):
    """Constructs a ResNet-12 model.
    """
    if drop:
        model = ResNet(BasicBlock, [1, 1, 1, 1], drop_rate=0.1, max_pool=max_pool,part=part, **kwargs)
    else:
        model = ResNet(BasicBlock, [1, 1, 1, 1], max_pool=max_pool, part=part,**kwargs)
    return model
